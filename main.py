...
def delete_information():
    key = request.form['key']  # Assuming the 'key' is passed through the form
    # Add logic to delete the entry from the database using the key
    try:
        information.delete(key)  # Assuming `delete()` removes the entry from the database
        return redirect(url_for('information_page'))  # Redirect back to the information page
    except Exception as e:
